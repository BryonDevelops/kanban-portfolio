...
export default function Board() {
  ...
  return (
    <div className="flex h-full flex-wrap gap-4 p-4 sm:p-6 overflow-x-hidden">
      {error && (
        <div className="w-full mb-4 p-3 rounded-md border border-red-400/30 bg-red-400/10 text-red-300">
          {error}
        </div>
      )}
        ...
        <DialogContent>
          ...
          <form
            onSubmit={async (e) => {
              ...
              // Use store method instead of service directly
              await addTask(quickAdd.colId, t, quickAdd.description.trim() || undefined);
              setOpen(false);
          ...
          >
            ...
            <DialogFooter>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="inline-flex items-center justify-center rounded-md border border-emerald-400/30 bg-emerald-400/10 px-3 py-1.5 text-sm font-medium text-emerald-200 hover:bg-emerald-400/15 disabled:opacity-50"
              >
