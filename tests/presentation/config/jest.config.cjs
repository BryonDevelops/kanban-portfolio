// jest.config.js for presentation layer tests
module.exports = {
  preset: 'ts-jest',
  ...
  rootDir: '../../../',
  testMatch: [
    '<rootDir>/tests/presentation/__tests__/**/*.(test|spec).(ts|tsx)',
  ],
  transform: {
    '^.+\\.(ts|tsx)$': ['ts-jest', {
      useESM: true,
  ...
  setupFilesAfterEnv: ['<rootDir>/tests/presentation/setup/jest.setup.ts'],
  moduleFileExtensions: ['ts', 'tsx', 'js', 'jsx', 'json'],
  collectCoverageFrom: [
    '<rootDir>/presentation/**/*.{ts,tsx}',
    '!<rootDir>/presentation/**/*.d.ts',
